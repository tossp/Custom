...
Diy_Part3() {
	...
	x86-64)
		if [[ "${REPO_URL}" == "https://github.com/coolsnowwolf/lede" ]];then
			...
			if [ -f "${Legacy_Firmware}" ];then
				...
				mv ${Legacy_Firmware} bin/Firmware/${AutoBuild_Firmware}-Legacy.${Firmware_sfx}
			...
			if [ -f "${EFI_Firmware}" ];then
				...
				mv ${EFI_Firmware} bin/Firmware/${AutoBuild_Firmware}-UEFI.${Firmware_sfx}
	...
	*)
		...
		mv ${Firmware_Path}/${Default_Firmware} bin/Firmware/${AutoBuild_Firmware}
